...
public class MateStoryListC {
	...
	public ModelAndView myStoryList(HttpServletRequest request) throws SQLException
	{
		...
		String mateCode = (String)request.getParameter("mateCode");
		...
		if(start<=count)		
		{		
			list = dao.getmyStory(Integer.parseInt(mateCode),start,end);
			mateList=dao.mateTagList();//친구 태그 불러오기	
			replyList = rDao.replyList();
			likeList = likeDAO.likeSearch(Integer.parseInt(mateCode));	// 명진 추가
			hateList = hateDAO.hateSearch(Integer.parseInt(mateCode));	// 명진 추가
