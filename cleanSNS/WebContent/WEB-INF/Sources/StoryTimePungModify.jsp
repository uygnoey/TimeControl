...
function serchMate()
{
	...
	window.name="parent";	
	window.open('pungmateadd.htm', 'child', style);
}

function confirm(code,name,mateName,postDate,photoURL,pungCode,postPhotoURL)
{
	...
	$.post("mystorypungmodifyresult.htm",{postCode:code,name:name,mateName:mateName,taglist:mates,taglistcode:matesCode,postDate:postDate,postContent:postContent,photoURL:photoURL,pungCode:pungCode,postPhotoURL:postPhotoURL},function(data){
...
function cancle(code,name,mateName,taglist,taglistcode,postDate,postContent,photoURL,pungCode,postPhotoURL)
{
	var photoURL2=photoURL.substring(3);
	photoURL="img\\"+photoURL2;
	
	$.post("mystorypungmodifyresult.htm",{postCode:code,name:name,mateName:mateName,taglist:taglist,taglistcode:taglistcode,postDate:postDate,postContent:postContent,photoURL:photoURL,pungCode:pungCode,postPhotoURL:postPhotoURL},function(data){
...
<form id="PumngModifyForm">
<div id="modify_${postCode}">
<table class="table table-simple table-hover" style="background:#B2EBF4;width:580px">
		...
		<td colspan="5" rowspan="7">
			...
			<td>
			<input type="button" value="확인" onclick="confirm('${postCode}','${name}','${mateName }','${postDate}','${photoURL }','${pungCode}','${postPhotoURL }')" style="float:right;background:#4374D9"class="postSubmit btn btn-black-gray btn-flat btn-rect" ></td>
			<td>
			<input type="button" value="취소" onclick="cancle('${postCode}','${name}','${mateName }','${taglist }','${taglistcode }','${postDate}','${postContent}','${photoURL }','${pungCode}','${postPhotoURL }')"  style="float:right;background:#4374D9"class="postSubmit btn btn-black-gray btn-flat btn-rect"  ></td>
			</tr>
			</table>
		</td>
	</tr>
